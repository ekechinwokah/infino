...
fn default_strategy_is_ingestion_time_with_one_day_granularity() {
    ...
    // partition_key is the 8-byte LE encoding of the ingestion-time bucket
    // (seconds/86400). The tag lives on each superfile entry (stamped by the
    // manifest's `update()` at commit); parts no longer carry a key of their own.
    let superfiles = m.get_all_superfiles();
    assert_eq!(superfiles.len(), 3);
    for sf in superfiles {
        assert_eq!(sf.partition_key.len(), PARTITION_KEY_BYTES);
    }
}
...
fn target_superfiles_per_partition_triggers_part_split() {
    ...
    // All superfiles belong to the same partition (split into 2 parts
    // by size), so they carry an identical partition_key. The tag lives
    // on each superfile entry now, not on the part.
    let superfiles = m.get_all_superfiles();
    assert_eq!(superfiles.len(), 3);
    assert!(
        superfiles
            .iter()
            .all(|sf| sf.partition_key == superfiles[0].partition_key),
...
fn time_range_assigns_int64_superfiles_to_bucket_zero() {
    ...
    // TimeRange partition_key is 8 bytes LE bucket index. The tag lives
    // on each superfile entry (stamped by the manifest's `update()`),
    // not on the part.
    let superfiles = m.get_all_superfiles();
