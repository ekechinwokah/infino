...
use crate::superfile::{
    ...
    vector::{
        ...
        distance::{
            ...
            encode_sq16_adaptive_row, encode_sq16_row, mean_f32_cluster_major, normalize,
            sq16_adaptive_norm_sq, sq16_decoded_norm_sq,
        },
    ...
    while rows_done < block.count {
        ...
        for row_idx in 0..take {
            ...
            if let (Some(res), Some(res_norm_bytes)) = (residual, res_norms.as_mut()) {
                // Decode the payload to fp32, then re-encode the 1-bit code
                // as the cluster-centroid residual's signs and record ‖r‖.
                // The payload is the authority: the estimator's
                // `q·c + κ·‖r‖·sd` then estimates the same vector the exact
                // rerank scores. Both FITTED arms above (Sq16Adaptive and the
                // fitted residual family) read the record's raw fp32 into
                // `fp_row` already; only the two verbatim-copy fixed arms
                // still need a decode here.
                if fixed_single_plane {
