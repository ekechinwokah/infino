...
impl FtsReader {
    ...
    /// Bound-ordered walk for a single bare phrase: admit the driver
    /// member's posting blocks best-first by a phrase score upper
    /// bound, verify positions only inside admitted doc windows via
    /// the windowed atom walk, and stop once no remaining window can
    /// beat the kth-best. This is what restores post-drain phrase
    /// pruning: a merged shard's correct global idf compresses scores
    /// into a band the doc-order walk can never get its bar above, so
    /// it verifies every candidate — visiting windows in bound order
    /// instead reaches the true top-k band after a few windows, and
    /// the rest of the shard prunes wholesale.
    ///
    /// The bound: for any doc in a window, `tf_phrase <= tf_member`
    /// for every member, so with `r_m` = (member's max block-max BM25
    /// over the window) / `idf_m_x_k1p1` — the best `tf/(tf+K1·norm)`
    /// ratio any doc in the window achieves for member `m` — the
    /// phrase score is at most `Σ_m idf_m_x_k1p1 × min_m r_m`.
    ///
    /// Windows are visited OUT of doc order, so the stop rule is
    /// strict (`ub < bar`): an equal-bound window may hold an
    /// equal-score doc with a smaller id that displaces the incumbent
    /// kth under the tie contract, and must be walked. Skipped docs
    /// are strictly below the local kth (or the monotonically rising
    /// global floor, which never exceeds the final kth), so the
    /// merged result matches the plain walk's.
    ///
    /// `Ok(None)` = declined (flat bounds, or admission degenerated
    /// past the bail thresholds) — the caller falls back to the plain
    /// walk. An absent member returns `Ok(Some([]))`: the phrase
    /// cannot match.
    pub(crate) async fn bm25_phrase_block_selected(
        ...
        let member_refs: Vec<&str> = phrase.iter().map(String::as_str).collect();
        let members = self
            .build_term_cursors_shared(column_id, &member_refs, cache)
            .await?;
        if members.len() != phrase.len() {
            return Ok(Some(Vec::new()));
        }
        // Driver = the member with the most blocks: finest doc
        // windows, hence the tightest per-window ratios from the
        // other members' coarser blocks.
        let driver = members
            .iter()
            .enumerate()
            .max_by_key(|(_, c)| c.blocks.len())
            .map(|(i, _)| i)
            .expect("members nonempty");
        let driver_blocks = Arc::clone(&members[driver].blocks);
        let total_blocks = driver_blocks.len();
        if total_blocks == 0 {
            return Ok(Some(Vec::new()));
        }
        let idf_phrase_x_k1p1: f32 = members.iter().map(|c| c.idf_x_k1p1).sum();

        // Per-driver-block phrase upper bound, in driver-block order.
        let mut order: Vec<(f32, u32)> = Vec::with_capacity(total_blocks);
        let mut window_lo = 0u32;
        for (b, blk) in driver_blocks.iter().enumerate() {
            let window_hi = blk.last_doc_id;
            let mut min_ratio = blk.block_max_bm25 / members[driver].idf_x_k1p1;
            for (m, cur) in members.iter().enumerate() {
                if m == driver {
                    continue;
                }
                let ratio =
                    member_span_max_bm25(&cur.blocks, window_lo, window_hi) / cur.idf_x_k1p1;
                min_ratio = min_ratio.min(ratio);
            }
            order.push((idf_phrase_x_k1p1 * min_ratio, b as u32));
            window_lo = window_hi.saturating_add(1);
        }
        let (lo_ub, hi_ub) = order
            .iter()
            .fold((f32::MAX, f32::MIN), |(lo, hi), &(ub, _)| {
                (lo.min(ub), hi.max(ub))
            });
        // Flat bounds order nothing — the ranged parallel walk wins.
        if hi_ub <= lo_ub {
            return Ok(None);
        }
        order.sort_unstable_by(|a, b| b.0.total_cmp(&a.0));

        let phrase_owned = phrase.to_vec();
        let mut heap: BinaryHeap<TopKEntry> = BinaryHeap::with_capacity(k);
        let mut admitted_total = 0usize;
        let mut wave_cap = BLOCK_SELECT_FIRST_WAVE;
        let mut next = 0usize;
        while next < order.len() {
            // Stop bar: the local kth (a future window's equal-score
            // doc may still displace on the smaller-id tie-break, so
            // the comparison below stays strict), the seeded floor,
            // and the live global floor (exact kth; strictly-below
            // docs can never reach the final merged top-k).
            let own = match heap.len() >= k {
                true => heap.peek().expect("heap len == k").0,
                false => f32::NEG_INFINITY,
            };
            let bar_stop = own
                .max(floor)
                .max(live.map(SharedFloor::get).unwrap_or(f32::NEG_INFINITY));
            if order[next].0 < bar_stop {
                break;
            }
            let wave_end = order.len().min(next + wave_cap);
            let mut wave: Vec<u32> = order[next..wave_end]
                .iter()
                .take_while(|(ub, _)| *ub >= bar_stop)
                .map(|&(_, b)| b)
                .collect();
            next += wave.len();
            admitted_total += wave.len();
            ...
            // Coalesce doc-adjacent admitted blocks into single
            // windows so one verify walk covers a contiguous run.
            wave.sort_unstable();
            ...
            for (b0, b1) in runs {
                let lo = match b0 {
                    0 => 0,
                    _ => driver_blocks[b0 as usize - 1].last_doc_id.saturating_add(1),
                };
                let hi_excl = driver_blocks[b1 as usize]
                    .last_doc_id
                    .checked_add(1)
                    .unwrap_or(u32::MAX);
                // Window admission floor: ties with the current kth
                // must be admitted (a smaller id displaces), so the
                // walk gets next_down of the effective bar.
                let admit = match heap.len() >= k {
                    true => heap.peek().expect("heap len == k").0.max(floor),
                    false => floor,
                };
                let atoms = self
                    .build_atom_cursors(column_id, &[], slice::from_ref(&phrase_owned), cache, lo)
                    .await?;
                let atoms: Vec<AnyCursor> = atoms.into_iter().flatten().collect();
                let window_hits = self.run_atoms_search(
                    column_id,
                    Vec::new(),
                    atoms,
                    k,
...
/// Max `block_max_bm25` over the blocks of `blocks` whose doc range
/// intersects `[lo, hi]`. Blocks partition ascending doc space — a
/// block's range is `(prev.last_doc_id, last_doc_id]` — so the scan
/// starts at the first block ending at/after `lo` and stops once a
/// block starts past `hi`. Returns `0.0` when nothing overlaps (no
/// member postings in the window ⇒ the phrase can't match there).
fn member_span_max_bm25(blocks: &[BlockMeta], lo: u32, hi: u32) -> f32 {
    let start = blocks.partition_point(|b| b.last_doc_id < lo);
    let mut max = 0.0f32;
    for i in start..blocks.len() {
        let block_start = match i {
            0 => 0,
            _ => blocks[i - 1].last_doc_id.saturating_add(1),
        };
        if block_start > hi {
            break;
        }
        max = max.max(blocks[i].block_max_bm25);
    }
    max
}
...
mod tests {
    ...
    /// The bound-ordered phrase kernel must agree with the plain
    /// phrase walk: same top-k docs and scores on a positional corpus
    /// with adjacency structure (members co-occur everywhere but are
    /// adjacent only in a subset), phrase tf variance (repeats in a
    /// sparse tier — the true top band), and dl variance, across k
    /// values spanning selective and near-exhaustive admission.
    #[tokio::test]
    async fn phrase_block_selected_matches_whole_walk() {
        let tok = Arc::new(AsciiLowerTokenizer);
        let mut b = FtsBuilder::new(tok);
        b.register_column("body".into(), true).expect("register");
        for d in 0..5000u32 {
            // Members co-occur non-adjacently in ~60% of docs (a
            // 100%-df member's block bounds all quantize to the
            // fixed-point floor and correctly decline the kernel), so
            // the candidate set is wide while matches are a subset.
            let mut text = String::new();
            if d % 5 < 3 {
                text.push_str("quick sep brown ");
                if d % 15 == 0 {
                    text.push_str("quick brown ");
                }
            }
            // High-tf tier CLUSTERED in one doc region so block maxes
            // vary (uniformly-spread repeats would flatten every
            // block's bound).
            if (1000..1250).contains(&d) && d % 5 == 0 {
                for _ in 0..(d % 7 + 2) {
                    text.push_str("quick brown ");
                }
            }
            text.push_str(&"filler ".repeat((d % 11) as usize));
            text.push_str(&format!("u{d}"));
            b.add_doc(0, d, &text).expect("add");
        }
        let blob = Bytes::from(b.finish().expect("finish"));
        let json = r#"[{"name":"body","tokenizer":"ascii_lower","positions":true}]"#;
            ...
            let expected = r
                .search_excluding(
                    ...
                    ClauseLists {
                        ...
                        negatives: &[],
                        must_phrases: &[],
                        should_phrases: slice::from_ref(&phrase),
                        negative_phrases: &[],
                    },
                    k,
                    f32::NEG_INFINITY,
                ...
                .expect("whole walk");
            let got = r
                .bm25_phrase_block_selected("body", &phrase, k, f32::NEG_INFINITY, None, None)
                .await
                .expect("kernel")
                .expect("bounds vary on this corpus — no decline");
            assert_eq!(got.len(), expected.len(), "k={k}");
            for (i, ((gd, gs), (ed, es))) in got.iter().zip(expected.iter()).enumerate() {
                assert_eq!(gd, ed, "k={k} doc at rank {i}");
                let rel = (gs - es).abs() / es.abs().max(f32::MIN_POSITIVE);
        ...
        // An absent member can never match: the kernel answers
        // directly instead of declining.
        let missing: Vec<String> = vec!["quick".into(), "nosuchterm".into()];
        let got = r
            .bm25_phrase_block_selected("body", &missing, 10, f32::NEG_INFINITY, None, None)
            .await
            .expect("kernel")
            .expect("absent member answers, not declines");
        assert!(got.is_empty());
    }
