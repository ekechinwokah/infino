...
pub(crate) mod test_util {
    ...
    pub(crate) fn synth_cells(n: usize, dim: usize) -> Vec<(Vec<f32>, u128)> {
        (0..n)
            .map(|i| {
                let c: Vec<f32> = (0..dim)
    ...
    pub(crate) fn clusters_from_cells(
        ...
        let flat: Vec<f32> = cells.iter().flat_map(|(c, _)| c.iter().copied()).collect();
        let ids: Vec<u128> = cells.iter().map(|(_, id)| *id).collect();
