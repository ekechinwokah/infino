...
/// probe range-GETs exactly the cluster), and its fp32 centroid (the k-means
/// center captured at the ingestion surface — never a decode of a stored
/// centroid). Every routing leaf is one internal IVF cluster: registration,
/// drain, and compaction all emit per-cluster leaves, so the tree routes
/// straight to clusters with no whole-cell leaf.
#[derive(Clone)]
...
mod tests {
    ...
    fn leaves_from(cells: &[(Vec<f32>, u128)]) -> Vec<LeafInsert> {
        cells
            .iter()
            .map(|(c, id)| LeafInsert {
                superfile_id: *id,
