...
impl CentroidTree {
    ...
    pub(crate) fn build(
        ...
        let root = build_subtree(metric, clusters, leaf_refs, &indices, &mut nodes, &mut sources);
        // Every node centroid IS an existing cell centroid (its source index),
        // sliced from `clusters` — no re-encode.
        let centroids = clusters.select_rows(&sources);
        Some(Self {
            centroids,
            nodes,
            root,
...
fn build_subtree(
    ...
    let children: Vec<u32> = groups
        .into_iter()
        .map(|g| build_subtree(metric, clusters, leaf_refs, &g, nodes, sources))
        .collect();
...
mod tests {
    ...
    fn build_has_one_leaf_per_cell_and_descends_all() {
        ...
        let cells = synth_cells(n, dim);
        {
            ...
            let all: HashSet<u128> = tree
                ...
                .collect();
            let want: HashSet<u128> = cells.iter().map(|(_, id)| *id).collect();
    ...
    fn select_leaves_bounded_and_finds_query_cell() {
        ...
        for metric in [Metric::L2Sq] {
            ...
            for &target in &probes_per {
                ...
                if probes
                    .iter()
                    .any(|(leaf, _)| leaf.superfile_id == cells[target].1)
                {
                    hits += 1;
                }
            }
    ...
    fn matches_flat_nearest_on_a_clustered_layout() {
        ...
        let dim = 16usize;
        let mut cells: Vec<(Vec<f32>, u128)> = Vec::new();
        let mut id = 1u128;
        ...
        for cluster in 0..n_queries {
            ...
            let mut flat: Vec<(u128, f32)> = cells
                .iter()
                .map(|(c, cid)| (*cid, distance(metric, &q, c)))
                .collect();
